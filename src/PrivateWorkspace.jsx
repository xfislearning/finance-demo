...
function CloudSettings({workspace}){
 ...
 const [accounts,setAccounts]=useState([]),[form,setForm]=useState(blank),[company,setCompany]=useState(workspace.companyName),[timezone,setTimezone]=useState(workspace.timezone||detectedTimezone),[message,setMessage]=useState('');
 async function load(){setAccounts(await listCloudAccounts());} useEffect(()=>{void load();},[]);
 async function addAccount(e){e.preventDefault();try{await createCloudAccount(workspace.organizationId,form);setForm(blank);setMessage('Account saved.');await load();}catch(err){setMessage(String(err));}}
 async function saveCompany(e){e.preventDefault();try{await updateOrganization(workspace.organizationId,{company_name:company,timezone});setMessage('Company settings updated. Refresh after saving to update the header/workspace settings.');}catch(err){setMessage(String(err));}}
 return <div className="split"><div><form className="panel form" onSubmit={saveCompany}><h3>Company</h3><Notice>{message}</Notice><Field label="Company name" value={company} onChange={e=>setCompany(e.target.value)}/><Field label="Country" value={workspace.country==='CA'?'Canada':'United States'} disabled/><Field label="Timezone" value={timezone} onChange={e=>setTimezone(e.target.value)} list="timezone-options"/><datalist id="timezone-options"><option value={detectedTimezone}/><option value="America/Denver"/><option value="America/New_York"/><option value="America/Chicago"/><option value="America/Los_Angeles"/><option value="America/Phoenix"/><option value="America/Anchorage"/><option value="Pacific/Honolulu"/><option value="America/Toronto"/><option value="America/Vancouver"/><option value="America/Edmonton"/><option value="America/Winnipeg"/><option value="America/Halifax"/><option value="America/St_Johns"/><option value="UTC"/></datalist><p className="muted">Automatically detected from this browser: <b>{detectedTimezone}</b>. System timestamps are stored in UTC and displayed using your company timezone. Bookkeeping dates are not shifted by timezone.</p><button type="button" onClick={()=>setTimezone(detectedTimezone)}>Use My Local Timezone</button><button className="primary">Save Company</button></form>
 <form className="panel form" onSubmit={addAccount}><h3>Add bank / payment account</h3><SelectField label="Account Type" value={form.account_type} onChange={v=>setForm({...form,account_type:v})} options={[["checking","Checking Account"],["savings","Savings Account"],["credit_card","Credit Card"],["debit_card","Debit Card"],["cash","Cash"],["payment_processor","Payment Processor"],["other","Other"]]}/><SelectField label="Used For" value={form.used_for} onChange={v=>setForm({...form,used_for:v})} options={[["business","Business"],["personal","Personal"]]}/><Field label="Financial Institution" value={form.institution_name} onChange={e=>setForm({...form,institution_name:e.target.value})}/><Field label="Nickname (optional)" value={form.nickname} onChange={e=>setForm({...form,nickname:e.target.value})}/><Field label="Last 4 Digits" maxLength="4" value={form.last_four} onChange={e=>setForm({...form,last_four:e.target.value.replace(/\D/g,'').slice(0,4)})}/><Field label="Currency" value={form.currency} disabled/><button className="primary">Save Account</button></form></div>
