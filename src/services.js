...
export async function readyForBatchReconciliation() {
  const bankRows = await listUnreconciledBank();
  const ready = [];
  for (const bank of bankRows) {
    const candidates = await reconciliationCandidates(bank);
    if (candidates.length !== 1) continue;
    const c = candidates[0];
    const dayDiff = Math.abs((new Date(`${bank.bank_date}T00:00:00Z`) - new Date(`${c.target_date}T00:00:00Z`)) / 86400000);
    if (dayDiff > 1) continue;
    ready.push({
      bank_transaction_id: bank.id,
      bank_date: bank.bank_date,
      bank_description: bank.description,
      bank_amount_cents: bank.amount_cents,
      target_type: c.target_type,
      target_id: c.id,
      target_date: c.target_date,
      target_label: c.label,
      amount_cents: c.amount_cents,
      confidence: dayDiff === 0 ? "High" : "High"
    });
  }
  return ready;
}

export async function approveBatchReconciliation(matches = []) {
  for (const m of matches) await reconcile(m.bank_transaction_id, m.target_type, m.target_id);
