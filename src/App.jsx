...
import {
  ...
  readyForBatchReconciliation,
  approveBatchReconciliation,
  listAllForBackup,
  getSettings,
  saveSettings,
  importExpenseRows,
...
function App() {
  ...
  async function initializeApp() {
    try {
      ...
      if (currentSession) {
        ...
        setSession(null); setWorkspace(null);
        await getSettings();
      }
      setReady(true);
  ...
  return <div className="app-frame">
    ...
    {authView && <AuthModal view={authView} onClose={() => setAuthView("")} onSwitch={setAuthView} onSignedIn={handleSignedIn} />}
  </div>;
}
...
function Page({ page, setPage, isDemo }) {
  let content;
  if (page === "Dashboard") content = <Dashboard setPage={setPage} isDemo={isDemo} />;
...
function Dashboard({ setPage, isDemo }) {
  ...
  return (
    <section>
      <GettingStarted setPage={setPage} isDemo={isDemo} />
...
function GettingStarted({ setPage, isDemo }) {
  if (isDemo) return <div className="panel onboarding demo-onboarding"><h3>Explore the demo</h3><p className="muted">Click through every module using sample data. Create an account when you want an empty private workspace for your own company.</p></div>;
  const steps = [
    ["Review company settings","Settings","Confirm company details and defaults."],
    ["Set up bank & payment accounts","Settings","Add the accounts you use for business spending and deposits."],
    ["Add or import expenses","Expenses","Scan, enter, or import expenses before reconciliation."],
    ["Reconcile transactions","Reconcile","Review automatic matches and approve them in batches."],
    ["Review reports","Reports","Review the Income Statement, Balance Sheet, Cash Flow, General Ledger, and YTD results."]
  ];
  return <div className="panel onboarding"><div className="panel-title-row"><div><h3>Getting Started</h3><p className="muted">Recommended bookkeeping flow. You can use any module at any time.</p></div><span className="step-count">5 steps</span></div><div className="onboarding-grid">{steps.map((x,i) => <button key={x[0]} onClick={() => setPage(x[1])}><b>{i+1}. {x[0]}</b><small>{x[2]}</small><span>Open →</span></button>)}</div></div>;
}
...
function Reconcile() {
  ...
  async function load(){
    try{
      ...
      const [bankRows, autoMatches]=await Promise.all([listUnreconciledBank(), readyForBatchReconciliation()]);
      setRows(bankRows);
      setReadyMatches(autoMatches);
      setSelectedReady(autoMatches.map((m)=>m.bank_transaction_id));
      if(selected){
  ...
  async function approveReady(){
    const matches=readyMatches.filter((m)=>selectedReady.includes(m.bank_transaction_id));
    if(!matches.length)return;
    try{await approveBatchReconciliation(matches);setMessage(`${matches.length} match${matches.length===1?"":"es"} approved and reconciled.`);setSelected(null);setCands([]);await load();}
    catch(err){setMessage(`Could not complete batch reconciliation: ${err}`);}
  }
  ...
  return <><div className="panel ready-reconcile"><div className="panel-title-row"><div><h3>Ready for Approval</h3><p className="muted">Qentro automatically prepares high-confidence bank-to-expense/payment matches. Review them, then approve in one batch.</p></div><b>{readyMatches.length} ready</b></div>{readyMatches.length===0?<p className="muted">No high-confidence matches are ready right now.</p>:<><div className="ready-match-list">{readyMatches.map((m)=><label key={m.bank_transaction_id} className="ready-match"><input type="checkbox" checked={selectedReady.includes(m.bank_transaction_id)} onChange={(e)=>setSelectedReady(e.target.checked?[...selectedReady,m.bank_transaction_id]:selectedReady.filter((id)=>id!==m.bank_transaction_id))}/><span><b>{m.bank_description}</b><small>{m.bank_date} → {m.target_label} ({m.target_date})</small></span><strong>{money(Math.abs(m.bank_amount_cents))}</strong><em>High confidence</em></label>)}</div><div className="button-row"><button onClick={()=>setSelectedReady(selectedReady.length===readyMatches.length?[]:readyMatches.map((m)=>m.bank_transaction_id))}>{selectedReady.length===readyMatches.length?"Clear All":"Select All"}</button><button className="primary" onClick={approveReady} disabled={!selectedReady.length}>Approve & Reconcile ({selectedReady.length})</button></div></>}</div><section className="split reconcile-layout">
    <div className="panel">
